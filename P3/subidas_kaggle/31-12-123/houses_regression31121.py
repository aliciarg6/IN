...
# y_pred = (y_pred_xgb + y_pred_lasso)/2 #submission-31-12-3
y_pred = 0.3*y_pred_xgb + 0.7*y_pred_lasso # submission-31-12-1
# y_pred = (y_pred_xgb + y_pred_lasso*0.9)/1.9 # submission-31-12-2
y_pred = np.exp(y_pred)

y_pred_train = xgbReg.predict(x_train)*0.3 + model_lasso.predict(x_train)*0.7
# y_pred_train = ( xgbReg.predict(x_train) + model_lasso.predict(x_train)*0.9 )/1.9
# y_pred_train = (xgbReg.predict(x_train) + model_lasso.predict(x_train))/2
get_score(y_pred_train,y_train)

outputName = 'submission_31-12-1.csv'
