...
XgbTest = xgb.XGBRegressor(n_estimators=3000).fit(x_train,y_train)
...
LGMTest = lgm.LGBMRegressor(n_estimators=3000).fit(x_train,y_train)
train_test(LGMTest,x_train,x_test,y_train,y_test)

scores = cross_val_score(LGMTest,train_features,train_labels,cv=5)
print("Accuracy: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))
...
lgm_model = LGMTest.fit(train_features,train_labels)


y_pred_train = ( GB_model.predict(train_features) + ENST_model.predict(train_features_st) ) /2
